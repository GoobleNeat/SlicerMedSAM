...
class MedSAMWidget(ScriptedLoadableModuleWidget, VTKObservationMixin):
    ...
    def setup(self):
        ...
        # Buttons
        self.ui.threeDModeButton.setCheckable(True)
    ...
    def onDeepGrowPointListNodeModified(self, observer, eventid):
        ...
        # 2. test/inference
        # 2.1 point [0, 3] need to be on the same slice
        for idx in range(1, 4):
            if points[idx][2] != points[0][2]:
                slicer.util.errorDisplay(
                    f"The {'first' if tdm else ''} four extreme points need to be in the same slice"
                )
        ...
        slice_idx = bb_points[0][2]
        xmin = max(min(bb_points[:, 1]) - 3, 0)
        xmax = min(max(bb_points[:, 1]) + 3, self.W)
